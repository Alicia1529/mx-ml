...
class SmoothedLDA(object):
    ...
    def _gibbs_sampler(self, n_gibbs, texts):
        ...
        for ii in range(self.N):
            ...
            C_dt[int(doc), int(assignments[ii, 0])] += 1
            C_wt[int(token_idx), int(assignments[ii, 0])] += 1
        ...
        for gg in range(n_gibbs):
            ...
            for jj in range(self.N):
                ...
                C_wt[int(token_idx), int(assignments[jj, gg])] -= 1
                C_dt[int(doc), int(assignments[jj, gg])] -= 1
