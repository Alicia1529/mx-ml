...
class FullyConnected(LayerBase):
    ...
    def _bwd2(self, dLdy, X, dLdy_bwd):
        ...
        dZ = self.act_fn.grad(np.dot(X, W) + b)
        ddZ = self.act_fn.grad2(np.dot(X, W) + b)

        ddX = np.dot(dLdy, W * dZ)
        ddW = np.dot(dLdy.T, (dLdy_bwd * dZ))
        ddB = np.sum(np.dot(dLdy,W) * dLdy_bwd * ddZ, axis=0, keepdims=True)
        return ddX, ddW, ddB
    ...
    def _bwd(self, dLdy, X):
        ...
        for dy, x in zip(dLdy, X):
            ...
            for dyi, xi in zip(*np.atleast_2d(dy, x)):
                ...
                dyidxi = np.diagflat(yi) - np.dot(yi, yi.T)  # jacobian wrt. input sample xi
                dxi.append(np.dot(dyi , dyidxi))
            dX.append(dxi)
